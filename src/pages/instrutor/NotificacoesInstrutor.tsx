...
import { notificacoesAPI, instrutorAPI } from '../../services/api'
...
export function NotificacoesInstrutor({ onNavigate: _onNavigate }: { onNavigate: (page: string) => void }) {
  const { C } = useTheme()

  const [todasNotificacoes, setTodasNotificacoes] = useState<any[]>([])
  const [turmaSetor,  setTurmaSetor]  = useState<string>('')
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      setTodasNotificacoes(data.notificacoes ?? [])
    } catch {
  ...
  useEffect(() => {
    ...
    instrutorAPI.minhaTurma().then((t: any) => {
      if (t) setTurmaSetor(t.cargo_grupo || t.nome || '')
    }).catch(() => {})
  }, [carregar])
  ...
  const notificacoesFiltradas = useMemo(() => {
    let lista = turmaSetor
      ? todasNotificacoes.filter(n => n.usuario_setor === turmaSetor)
      : todasNotificacoes
    if (filtroTipo !== 'todos') {
      lista = lista.filter(n => n.tipo === filtroTipo)
    }
    return lista
  }, [todasNotificacoes, filtroTipo, turmaSetor])

  const total        = notificacoesFiltradas.length
  const totalPaginas = Math.max(1, Math.ceil(total / LIMITE_PAGINA))
  const notificacoes = notificacoesFiltradas.slice((pagina - 1) * LIMITE_PAGINA, pagina * LIMITE_PAGINA)
