...
function CapaLivroSVG({ mat, mini = false }: { mat: Material; mini?: boolean }) {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} xmlns="http://www.w3.org/2000/svg" style={{ display: 'block' }}>
      <defs>
        <linearGradient id={`grad-col-${mat.id}`} x1="0" y1="0" x2="1" y2="1">
      ...
      {/* Capa principal */}
      <rect x={mini ? 2 : 4} y={mini ? 2 : 4} width={W - (mini ? 12 : 24)} height={H - (mini ? 8 : 16)} rx={mini ? 3 : 6} fill={`url(#grad-col-${mat.id})`} />
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: 'auto' }}>
              <button
                ...
                style={{
                  ...
                  background: favoritado ? 'rgba(245,158,11,0.10)' : 'none',
                  border: `1.5px solid ${favoritado ? '#f59e0b' : C.border}`,
                  borderRadius: '8px', padding: '10px',
                  fontSize: '13px', fontWeight: 500,
                  color: favoritado ? '#f59e0b' : C.text,
...
interface ApostilasColaboradorProps {
...
export function ApostilasColaborador({ onNavigate, onLogout }: ApostilasColaboradorProps) {
  ...
  const limparFiltros = () => {
    ...
    setTipoFiltro('Todos'); setApenasDestaques(false); setPaginaAtual(1)
  }
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: isSmall ? '16px' : '32px 40px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
            <div>
              ...
              <h1 style={{ fontSize: '22px', fontWeight: 700, color: C.text, margin: '0 0 4px' }}>Apostilas e Materiais</h1>
              <p style={{ fontSize: '13px', color: C.muted, margin: 0 }}>
                {materiaiFiltrados.length} de {totalDisponiveis} materiais disponíveis
              </p>
            </div>
            <div style={{ display: 'flex', background: C.surface, border: `1px solid ${C.border}`, borderRadius: '8px', overflow: 'hidden' }}>
          ...
          {visualizacao === 'lista' && materiaiFiltrados.length > 0 && (
            <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '12px', overflow: 'hidden' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '44px 2fr 1fr 80px 80px 80px 80px', background: C.surface2, borderBottom: `1px solid ${C.border}` }}>
                {['', 'Título', 'Autor', 'Tipo', 'Páginas', 'Downloads', 'Favoritos'].map((h, i) => (
                  <div key={i} style={{ padding: '12px', fontSize: '11px', fontWeight: 700, color: C.muted, textTransform: 'uppercase', letterSpacing: '0.5px', borderRight: i < 6 ? `1px solid ${C.border}` : 'none' }}>
                    {h}
                  </div>
                ))}
              </div>
              {materiaiFiltrados.map((mat, idx) => (
                <div
                  key={mat.id}
                  onClick={() => setModalMat(mat)}
                  style={{ display: 'grid', gridTemplateColumns: '44px 2fr 1fr 80px 80px 80px 80px', borderBottom: idx < materiaiFiltrados.length - 1 ? `1px solid ${C.border}` : 'none', cursor: 'pointer', transition: 'background 150ms' }}
                ...
                >
                  ...
                  <div style={{ padding: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
